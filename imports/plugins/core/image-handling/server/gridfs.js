...
// TODO: What is share.defaultRoot ??
if (Meteor.isServer) {
  export class FileCollection extends Mongo.Collection {
    constructor(root = share.defaultRoot, options = {}) {
      ...
      if (optionsObj.resumable || optionsObj.http) {
        share.setupHttpAccess.bind(this)(optionsObj);
      }
    ...
    insert(file = {}, callback = undefined) {
      const superFile = share.insert_func(file, this.chunkSize);
    ...
    update(selector, modifier, options = {}, callback = undefined) {
      ...
      if (share.reject_file_modifier(modifier) && !optionsObj.force) {
    ...
    upsertStream(file, options = {}, callback = undefined) {
      ...
      callbackFunction = share.bund_env(callbackFunction);
    ...
    remove(selector, callback = undefined) {
      const callbackFunction = share.bind_env(callback);
    ...
    importFile(filePath, file, callback) {
      ...
      readStream.on("error", share.bind_env(callbackFunction));
      readStream.pipe(share.streamChunker(this.chunkSize)).pipe(writeStream)
        .on("close", share.bind_env(function (d) {
          callbackFunction(null, d);
        }))
        .on("error", share.bind_env(callbackFunction));
    }
    ...
    exportFile(selector, filepath, callback) {
      ...
      readStream.pipe(writeStream)
        .on("finish", share.bind_env(callbackFunction))
        .on("error", share.bind_env(callbackFunction));
    }
  }
}
